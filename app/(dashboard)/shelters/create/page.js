...
export default function CreateShelterPage() {
  ...
  useEffect(() => {
    const fetchVolcanoes = async () => {
      try {
        const { data, error } = await shelterService.getVolcanoes()
        ...
        if (filtered.length > 0) {
          // AUTO SELECT the first (and only) volcano
          setFormData(prev => ({ ...prev, volcano_id: filtered[0].id }))
        }
      } catch (err) {
        console.error(err)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await shelterService.create(dataToSave)
      
      if (error) throw error

      toast.success("Titik evakuasi berhasil ditambahkan")
  ...
  return (
    <div className="min-h-screen p-6 md:p-10 max-w-4xl mx-auto font-sans">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Form Data Evakuasi</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Lengkapi informasi di bawah untuk menambahkan lokasi operasional baru.</p>
      </div>
        ...
        <div className="p-6 md:p-8 space-y-8">
            ...
            <div className="space-y-1.5">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center justify-between">
                  ...
                  {volcanoes.length === 1 && (
                    <span className="text-[10px] bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter">
                      Otomatis
                    </span>
                  )}
                </label>
            ...
            {/* Field Address */}
            <div className="space-y-1.5 md:col-span-2">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Alamat Lengkap (Otomatis dari Pin Peta)</label>
                <textarea 
                name="address"
                ...
                placeholder="Geser pin pada peta untuk mengisi alamat..."
                />
            </div>
